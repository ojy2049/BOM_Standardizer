...
### 5. 부품 라이브러리 및 별칭 관리
- **부품 DB**: SQLite 기반 부품 라이브러리 (MPN, 제조사, 패키지, 별칭 관리)
- **부품별 별칭**: 동일 부품의 다양한 표기 통합 (여러 개 등록 가능)
...
- **BOM-Centroid 매칭**: BOM과 좌표 데이터 자동 매칭 및 누락 검사
...
### v2.2.3 (2026-01-26) 🔧 NPI/P&P 매칭 개선
- **RefDes 정규화**: BOM과 Centroid의 RefDes 형식 차이 자동 보정
  - `R011` → `R11`, `C025` → `C25` 등 앞자리 0 자동 제거
  - 다른 CAD 툴 간 호환성 향상
- **매칭 결과 저장 버튼 추가**: BOM+Centroid 매칭 데이터를 Excel/CSV로 저장
- **Layer 값 처리 개선**: None/NaN 값 시 기본값 'Top' 적용하여 P&P 파일 생성 오류 해결
- **디버그 로그 추가**: 매칭 실패 시 원인 파악을 위한 샘플 RefDes 출력

### v2.2.2 (2026-01-19) 🛠️ GUI 개선
- **탭 구조 개선**: 5개 탭으로 재구성 (BOM 입력 → 처리 결과 → SMT 장착 → 부품 DB → 설정)
  - "NPI / P&P" 탭을 "SMT 장착" 탭으로 통합
  - "별칭 관리" 탭을 "부품 DB" 탭으로 통합
  - "설정" 탭 항상 마지막 위치
- **부품 라이브러리 별칭 통합**:
  - 부품 테이블에 "별칭" 컨럼 추가
  - 부품 편집 다이얼로그에 "별칭" 필드 추가 (여러 개 입력 가능)
  - AliasManager 연동으로 별칭 저장/로드
- **NPI Pro 다이얼로그 버튼 추가**: 프로젝트 생성/버전 저장 다이얼로그에 "취소" 버튼 추가
